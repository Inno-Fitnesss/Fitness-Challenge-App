...
const EXERCISES = {
  ...
  pushup: {
    ...
    settings: [
      { key: "bottom", label: "Нижняя фаза", min: 65, max: 130, step: 1, value: 105, suffix: "°" },
      { key: "top", label: "Верхняя фаза", min: 140, max: 178, step: 1, value: 155, suffix: "°" },
      { key: "bodyLine", label: "Минимальный угол корпуса", min: 135, max: 178, step: 1, value: 150, suffix: "°" },
    ],
  },
...
function updateRepState(primaryAngle) {
  ...
  if (repState === "WAITING_FOR_TOP") {
    ...
    if (stableTopFrames >= MIN_STABLE_FRAMES) {
      repState = "READY";
  ...
  if (repState === "READY") {
    ...
    if (stableBottomFrames >= MIN_STABLE_FRAMES) {
...
function analyzePushup(measurement) {
  ...
  const maxTilt = pushupVariant === "knees" ? 50 : 45;
