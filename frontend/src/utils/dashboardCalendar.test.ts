...
describe("buildWeekDays — today's partial vs full completion (flame source)", () => {
  // Multi-exercise / multi-challenge "today" builders (the module-level
  // makeTodayChallenge only covers a single closed/open exercise).
  function todayChallenge(id: number, closedFlags: boolean[]): ApiTodayChallenge {
    return {
      id,
      name: `Challenge ${id}`,
      exercises: closedFlags.map((closed, i) => ({
        challenge_exercise_id: i + 1,
        exercise_id: i + 1,
        name: 'Squats',
        metric: 'reps' as const,
        goal: 10,
        clean_today: closed ? 10 : 0,
        closed,
      })),
    };
  }

  function freezeJune3() {
    vi.useFakeTimers();
    vi.setSystemTime(new Date(2026, 5, 3, 12, 0, 0)); // "today" = Wed June 3
    return getWeekStart(new Date(2026, 5, 3));
  }

  it('all exercises closed today → "full"', () => {
    const weekStart = freezeJune3();
    const days = buildWeekDays(weekStart, [makeChallenge()], [], [todayChallenge(1, [true, true, true])]);
    const today = days.find((d) => d.isoDate === '2026-06-03');
    expect(today?.status).toBe('full');
  });

  it('a single challenge with SOME (not all) exercises done today is NOT "full"', () => {
    // Calendar completion is tracked per-CHALLENGE, not per-exercise: a single
    // challenge only counts once ALL its exercises are closed. So 2-of-3 done
    // reads as "pending" (the challenge isn't finished), and — crucially — it
    // must never read as "full".
    const weekStart = freezeJune3();
    const days = buildWeekDays(weekStart, [makeChallenge()], [], [todayChallenge(1, [true, true, false])]);
    const today = days.find((d) => d.isoDate === '2026-06-03');
    expect(today?.status).toBe('pending');
    expect(today?.status).not.toBe('full');
  });

  it('one closed exercise out of three today → "pending" (challenge not finished)', () => {
    const weekStart = freezeJune3();
    const days = buildWeekDays(weekStart, [makeChallenge()], [], [todayChallenge(1, [true, false, false])]);
    const today = days.find((d) => d.isoDate === '2026-06-03');
    expect(today?.status).toBe('pending');
  });

  it('nothing closed today → "pending"', () => {
    const weekStart = freezeJune3();
    const days = buildWeekDays(weekStart, [makeChallenge()], [], [todayChallenge(1, [false, false, false])]);
    const today = days.find((d) => d.isoDate === '2026-06-03');
    expect(today?.status).toBe('pending');
  });

  it('empty exercise list today is NOT treated as complete (every([]) guard)', () => {
    const weekStart = freezeJune3();
    const emptyToday: ApiTodayChallenge = { id: 1, name: 'Empty', exercises: [] };
    const days = buildWeekDays(weekStart, [makeChallenge()], [], [emptyToday]);
    const today = days.find((d) => d.isoDate === '2026-06-03');
    expect(today?.status).toBe('pending');
  });

  it('two scheduled challenges, only one fully done today → "partial"', () => {
    const weekStart = freezeJune3();
    const c1 = makeChallenge({ id: 1 });
    const c2 = makeChallenge({ id: 2 });
    const days = buildWeekDays(
      weekStart,
      [c1, c2],
      [],
      [todayChallenge(1, [true]), todayChallenge(2, [false])],
    );
    const today = days.find((d) => d.isoDate === '2026-06-03');
    expect(today?.status).toBe('partial');
  });

  it('two scheduled challenges, both fully done today → "full"', () => {
    const weekStart = freezeJune3();
    const c1 = makeChallenge({ id: 1 });
    const c2 = makeChallenge({ id: 2 });
    const days = buildWeekDays(
      ...
      [c1, c2],
      [],
      [todayChallenge(1, [true]), todayChallenge(2, [true])],
    );
    const today = days.find((d) => d.isoDate === '2026-06-03');
    expect(today?.status).toBe('full');
  });
